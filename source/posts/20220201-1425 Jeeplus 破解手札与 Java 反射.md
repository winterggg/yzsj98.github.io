...
继续阅读了包里的其他反汇编代码，最后定位到了生成器的核心源码 `D8.class` 。该文件包含了 license 验证（调用之前的 `C7.class`）和代码生成器的功能实现代码。想吐槽一下原来这里的代码生成器就是字符串拼接，直接在代码里拼接的那种...
...
这里也能印证之前的猜测。值得一提的是，我也有尝试过反汇编 `D8.class` 直接去掉验证逻辑，然而不知道是因为这个文件代码量太大（1500多行），还是因为代码做了混淆，反汇编的代码会有一大堆难以解决的语法错误，因担心暴力修改会破坏生成器的功能，遂放弃。
...
其中第三行使用 `Class::getDeclaredField` 获取待修改的 Field 成员，由于该成员是 private 的。第四行使用 `setAccessible` 设置权限。5~7行使用了位操作的技巧关闭了 Final 标识。第 8 行设置新值。
